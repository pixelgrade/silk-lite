...
if ( ! class_exists('Swell_Popular_Posts_Widget') ) :
	...
	class Swell_Popular_Posts_Widget extends WP_Widget {

		var $alt_option_name = 'widget_swell_popularposts';
		var $default_title = '';

		function __construct() {
			parent::__construct(
				'swell-popular-posts',
				apply_filters( 'swell_widget_name', __( 'Swell Popular Posts', 'swell_txtd' ) ),
				array(
					'description' => __( 'Shows your most viewed posts.', 'swell_txtd' ),
				)
			);

			$this->default_title =  __( 'Popular Posts', 'swell_txtd' );
		}
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:', 'swell_txtd' ); ?></label>
			<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'count' ); ?>"><?php esc_html_e( 'Maximum number of posts to show (no more than 10):', 'swell_txtd' ); ?></label>
		...
		<p><?php esc_html_e( 'Popular Posts by views are calculated from 24-48 hours of stats. They take a while to change.', 'swell_txtd' ); ?></p>
		...
		function widget( $args, $instance ) {
			...
			$count = apply_filters( 'swell_popular_posts_widget_count', $count );
			...
			$get_image_options = apply_filters( 'swell_popular_posts_widget_image_options', $get_image_options );
			...
			if ( !$posts ) {
				if ( current_user_can( 'edit_theme_options' ) ) {
					echo '<p>' . sprintf(
							__( 'There are no posts to display. <a href="%s">Want more traffic?</a>', 'swell_txtd' ),
			...
			<li class='popular-posts_item large-thumbnail'>
				<?php do_action( 'swell_widget_popular_posts_before_post', $post['post_id'] ); ?>
				...
				<?php
					$cats_list = swell_get_cats_list($post['post_id']);
				...
				<?php do_action( 'swell_widget_popular_posts_after_post', $post['post_id'] ); ?>
			</li>
			<?php
